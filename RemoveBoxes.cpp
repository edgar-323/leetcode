...
private:
    ...
    int recurse1(const int i, const int j, const int k, const std::vector<int>& boxes) {
        if (i > j) {
            return 0;
        } else if (cache1[i][j][k] >= 0) {
            return cache1[i][j][k];
        }

        int max_pts = (k+1) * (k+1) + recurse1(i+1, j, 0, boxes);
        for (int m = i+1; m <= j; ++m) {
            if (boxes[m] == boxes[i]) {
                max_pts = std::max(max_pts, recurse1(i+1, m-1, 0, boxes) + recurse1(m, j, k+1, boxes));
            }
        }

        return cache1[i][j][k] = max_pts;
    }
	int solution1(const std::vector<int>& boxes) {
        /* Time-Complexity:     O(N^4)
         * Space-Complexity:    O(N^3)
         */
		/* EXPLANATION (Credit Goes To LeetCoder @fun4LeetCode):
			...
			Let the input array be boxes with length n. Define T(i, j) as the maximum points one can get by removing boxes of the subarray boxes[i, j] (both inclusive). 
            ...
            Take the first box boxes[i](i.e., the box at index i) as an example. 
            ...
            But still you want maximum points for the remaining boxes, which by definition is T(i + 1, j). 
            In total your points will be 1 + T(i + 1, j).

			But later after reading the rules more carefully, you realize that you might get more points if this box (boxes[i]) can be removed together with other boxes of the same color. 
			...
			At this moment, the boxes we left behind consist of two parts: the one at index i (boxes[i]) and those of the subarray boxes[m, j], with the former bordering the latter from the left. 
            Apparently there is no way applying the definition of the subproblem to the subarray boxes[m, j], since we have some extra piece of information that is not included in the definition. 
			...
			Another example of problem that does not have self-contained subproblems is leetcode 312. Burst Balloons, 
            where the maximum coins of subarray nums[i, j] depend on the two numbers adjacent to nums[i] on the left and to nums[j] on the right. 
            So you may find some similarities between these two problems.

			Problems without self-contained subproblems usually don't have well-defined recurrence relations, which renders it impossible to be solved recursively. 
            The cure to this issue can sound simple and straightforward: modify the definition of the problem to absorb the external information so that the new one is self-contained.

			So let's see how we can redefine T(i, j) to make it self-contained. 
            First let's identify the external information. 
            On the one hand, from the point of view of the subarray boxes[m, j], it knows nothing about the number (denoted by k) of boxes of the same color as boxes[m]to its left. 
            ...
            Next let's absorb this extra piece of information into the definition of T(i, j) and redefine it as T(i, j, k), 
            which denotes the maximum points possible by removing the boxes of subarray boxes[i, j] with k boxes attached to its left of the same color as boxes[i]. 
			...
			b.  If we decide to attach boxes[i] to some other box of the same color, say boxes[m], then from our analyses above, 
                the total points will be T(i + 1, m - 1, 0) + T(m, j, k + 1), where for the first term, 
                since there is no attached boxes for subarray boxes[i + 1, m - 1], we have k = 0 for this part; 
                ...
                Therefore the final answer for this case will be: max(T(i + 1, m - 1, 0) + T(m, j, k + 1)) where i < m <= j && boxes[i] == boxes[m].
            ...
            Here each subproblem is characterized by three integers (i, j, k), all of which are bounded, 
            i.e, 0 <= i, j, k < n, so a three-dimensional array (n x n x n) will be good enough for the cache.
			...
			Side notes: 
            In case you are curious, for the problem "leetcode 312. Burst Balloons", 
            the external information to subarray nums[i, j] is the two numbers (denoted as left and right) adjacent to nums[i] and nums[j], respectively. 
            If we absorb this extra piece of information into the definition of T(i, j), we have T(i, j, left, right), 
            which represents the maximum coins obtained by bursting balloons of subarray nums[i, j] whose two adjacent numbers are left and right. 
            The original problem will be T(0, n - 1, 1, 1) and the termination condition is T(i, i, left, right) = left * right * nums[i]. 
            The recurrence relations will be: T(i, j, left, right) = max(left * nums[k] * right + T(i, k - 1, left, nums[k]) + T(k + 1, j, nums[k], right)) 
            where i <= k <= j 
	...
	*/ 
        ...
        N = boxes.size();
        init_cache1();
        const int max_pts = recurse1(0, N-1, 0, boxes);
        destroy_cache1();
